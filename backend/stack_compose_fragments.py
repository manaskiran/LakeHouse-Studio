...
# P0.8 — Superset build context. A Trivy scan of the stock upstream image
# turned up 583 OS-package CVEs (9 critical, 361 high) with a patch already
# released by Debian — fixable with nothing more than an apt-get upgrade
# layered on top. Rather than leave that as a manual "rebuild yourself"
# follow-up, every Startup Analytics install now builds this hardened
# variant instead of pulling the stock tag directly.
_SUPERSET_BASE_IMAGE = "apache/superset:4.1.1"
_SUPERSET_BUILD_DIR = "superset-hardened"
_SUPERSET_DOCKERFILE = (
    f"# Auto-generated by backend/stack_compose_fragments.py (P0.8).\n"
    f"# Rebuilds {_SUPERSET_BASE_IMAGE} with current Debian OS-package\n"
    f"# security patches layered on top. Re-run `docker compose build superset`\n"
    f"# periodically to pick up newly released patches for this base image.\n"
    f"FROM {_SUPERSET_BASE_IMAGE}\n"
    f"USER root\n"
    f"RUN apt-get update && apt-get upgrade -y --no-install-recommends \\\n"
...
def _render_superset_fragment(env: dict) -> str:
    ...
    return (
        ...
        "services:\n"
        "  superset:\n"
        # P0.8 — build the OS-patched variant instead of pulling the stock
        # tag; see _SUPERSET_DOCKERFILE above and write_fragment() below,
        # which drops the Dockerfile into ./superset-hardened alongside
        # this fragment. `docker compose up` builds it automatically on
        # first run (no cached image tag yet); re-running the same command
        # after a rebuild picks up the cached layer, same as any other
        # compose service.
        f"    build:\n"
        f"      context: ./{_SUPERSET_BUILD_DIR}\n"
        f"      dockerfile: Dockerfile\n"
        f"    image: udp-superset-hardened:4.1.1\n"
...
def write_fragment(stack_id: str, install_dir: Path,
    ...
    # P0.8 — Startup Analytics builds a Debian-patched Superset instead of
    # pulling the stock tag; drop the Dockerfile the fragment's `build:`
    # directive points at.
    if stack_id == "startup-analytics-local-v0.1":
        dockerfile_path = install_dir / _SUPERSET_BUILD_DIR / "Dockerfile"
        _atomic_write(dockerfile_path, _SUPERSET_DOCKERFILE)
        log.info("wrote hardened Superset Dockerfile for stack=%s", stack_id)
    log.info(
